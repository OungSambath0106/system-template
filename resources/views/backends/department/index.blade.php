                                        ...
                                        <div class="col-sm-6 filter">
                                            <select name="compus_id" id="compus_id" class="form-control select2">
                                                <option value="" class="form-control"
                                                    {{ !request()->filled('compuses') ? 'selected' : '' }}>
                                                    {{ __('All Campus') }}
                                                </option>
                                                @foreach ($compuses as $compus)
                                                    <option value="{{ $compus->id }}" class="form-control"
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row align-items-center">
                                ...
                                <div class="col-sm-6">
                                    <a class="btn btn-primary float-right" href="{{ route('admin.department.create') }}">
                        ...
                        {{-- table --}}
                        @include('backends.department._table')
...
@push('js')
    <script>
        ...
        $('input.status').on('change', function() {
            ...
            $.ajax({
                type: "get",
                url: "{{ route('admin.department.update_status') }}",
                data: {
                    "id": $(this).data('id')
                },
                dataType: "json",
    ...
    <script>
        $(document).ready(function() {
            $(document).on('change', '#compus_id', function(e) {
                e.preventDefault();
