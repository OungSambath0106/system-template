...
<div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content">
        ...
        <form action="{{ route('admin.setting.language.update', ['id' => $lang['id']]) }}" class="submit-form" method="post">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="name" class="form-control" value="{{ $lang['name'] }}" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="code" id="code" class="form-control country_code_select">
                                @foreach(\Illuminate\Support\Facades\File::files(base_path('public/backend/plugins/flag-icon-css/flags/4x3')) as $path)
                                    @if(pathinfo($path)['filename'] !='gb')
                                        <option value="{{ pathinfo($path)['filename'] }}" title="{{ asset('backend/plugins/flag-icon-css/flags/4x3/'.pathinfo($path)['filename'].'.svg') }}" {{ $lang['code'] == pathinfo($path)['filename'] ? 'selected' : '' }}>
                                            {{ strtoupper(pathinfo($path)['filename']) }}
                                        </option>
                                    @else
                                        <option value="en" title="{{ asset('backend/plugins/flag-icon-css/flags/4x3/'.pathinfo($path)['filename'].'.svg') }}" {{ $lang['code'] == 'en' ? 'selected' : '' }}>
                                            {{ strtoupper('en') }}
                                        </option>
                                    @endif
                                @endforeach
